...
function GetMeanGrade(Originaldata){
	...
	classRoot.on('child_added', function(snapshot) {
		...
		document.getElementById(classID[index]).children[2].innerHTML = "<strong>Professor</strong>: " + snapshot.child("Info/Professor").val();
		document.getElementById(classID[index]).children[3].innerHTML = "<strong>Email</strong>: " + snapshot.child("Info/Email").val();
		document.getElementById(classID[index]).children[4].innerHTML = "<strong>Lecture</strong>: " + snapshot.child("Info/Lecture").val();
		document.getElementById(classID[index]).children[5].innerHTML = "<strong>Location</strong>: " + snapshot.child("Info/Location").val();
		document.getElementById(classID[index]).children[6].innerHTML = "<strong>Office</strong>: " + snapshot.child("Info/Office").val();
		document.getElementById(classID[index]).children[7].innerHTML = "<strong>Office Hour</strong>: " + snapshot.child("Info/Office Hour").val();

		index++;
	});

	CS188_data.once('value', function(snap){
    ...
    $('#calendar').fullCalendar({
        ...
        dayClick: function(date) {
            // Change the date for daily list
            $('#dailyCal').fullCalendar('gotoDate', date.format());
            //$('#dailyCal').fullCalendar('getDate').format();
            //$('#dailyCal').currentDate=date.format();
            //$('#dailyCal').renderView();
            $('#calendar').fullCalendar('select', date);
            //$(".fc-highlight").css("background", "red")
            $('.dayRender', function(date, cell){
            	//$('.fc-day[data-date="' + date + '"]').css('background', red);
